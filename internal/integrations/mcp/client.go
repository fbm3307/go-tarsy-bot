...
func (c *MCPClientImpl) Connect(ctx context.Context, serverConfig *ServerConfig) error {
	...
	c.logger.Info("📦 Step 1: Starting MCP server process...", zap.String("server", serverConfig.Name))
	if err := c.startServerProcess(ctx, serverConfig); err != nil {
		return fmt.Errorf("failed to start server process: %w", err)
	}
	c.logger.Info("✅ Step 1 complete: Process started", zap.String("server", serverConfig.Name))

	// Start message processing goroutines
	c.logger.Info("📡 Step 2: Starting message processing goroutines...", zap.String("server", serverConfig.Name))
	go c.readMessages()
	go c.processMessages()
	
	// Give it a moment to initialize
	time.Sleep(500 * time.Millisecond)
	c.logger.Info("✅ Step 2 complete: Message processors started", zap.String("server", serverConfig.Name))

	// Perform MCP handshake
	c.logger.Info("🤝 Step 3: Performing MCP handshake...", zap.String("server", serverConfig.Name))
	if err := c.performHandshake(ctx); err != nil {
		c.logger.Error("❌ Handshake failed", zap.String("server", serverConfig.Name), zap.Error(err))
		c.cleanup()
		return fmt.Errorf("handshake failed: %w", err)
	}
	c.logger.Info("✅ Step 3 complete: Handshake successful", zap.String("server", serverConfig.Name))

	// Discover available tools
	c.logger.Info("🔍 Step 4: Discovering available tools...", zap.String("server", serverConfig.Name))
	if err := c.discoverTools(ctx); err != nil {
		c.logger.Error("❌ Tool discovery failed", zap.String("server", serverConfig.Name), zap.Error(err))
		c.cleanup()
		return fmt.Errorf("tool discovery failed: %w", err)
	}
	c.logger.Info("✅ Step 4 complete: Tools discovered", zap.String("server", serverConfig.Name), zap.Int("tool_count", len(c.tools)))
...
func (c *MCPClientImpl) startServerProcess(ctx context.Context, config *ServerConfig) error {
	...
	// Don't use context for the command itself - MCP servers run indefinitely
	// The context is only for initialization timeout
	c.cmd = exec.Command(config.Command, args...)
	...
	c.logger.Info("✅ MCP server process started successfully",
	...
	go func() {
		...
		for scanner.Scan() {
			line := scanner.Text()
			if line != "" {
				c.logger.Debug("MCP stderr", zap.String("line", line))
			}
		}
	}()
...
func (c *MCPClientImpl) readMessages() {
	c.logger.Info("📖 Starting message reader goroutine")
	...
	for scanner.Scan() {
		...
		messageCount++
		c.logger.Debug("📨 Received message from MCP server",
...
func (c *MCPClientImpl) processMessages() {
	c.logger.Info("🔄 Starting message processor goroutine")
	processedCount := 0
	for {
		select {
		case msg := <-c.messageBuffer:
			...
			c.handleMessage(msg)
		case <-c.ctx.Done():
			c.logger.Info("Message processor stopping", zap.Int("total_processed", processedCount))
			return
		}
	}
}
...
func (c *MCPClientImpl) handleMessage(msg *MCPMessage) {
	...
	c.logger.Debug("🎯 Handling message",
		zap.Any("id", msg.ID),
		zap.String("method", msg.Method),
		zap.Bool("has_result", msg.Result != nil),
		zap.Bool("has_error", msg.Error != nil))

	// Check if this is a response to a pending request
	if msg.ID != nil {
		if ch, exists := c.pendingRequests[msg.ID]; exists {
			c.logger.Info("✅ Delivering response to pending request", zap.Any("id", msg.ID))
			delete(c.pendingRequests, msg.ID)
			select {
			case ch <- msg:
				c.logger.Debug("Response delivered successfully", zap.Any("id", msg.ID))
			default:
				c.logger.Warn("Failed to deliver response - channel full or closed", zap.Any("id", msg.ID))
			}
			return
		} else {
			c.logger.Warn("⚠️  Received response for unknown request ID", zap.Any("id", msg.ID))
		}
	}

	// Handle notifications and requests from server
	if msg.Method != "" {
		c.logger.Debug("Handling server notification", zap.String("method", msg.Method))
...
func (c *MCPClientImpl) sendRequest(ctx context.Context, method string, params interface{}) (*MCPMessage, error) {
	...
	c.logger.Info("📤 Sending MCP request",
	...
	c.logger.Debug("⏳ Waiting for response...", zap.Int("request_id", id))

	select {
	case resp := <-respChan:
		c.logger.Info("📥 Received MCP response",
			zap.String("method", method),
			zap.Int("request_id", id),
	...
	case <-ctx.Done():
		...
		delete(c.pendingRequests, id)
		c.mutex.Unlock()
		c.logger.Error("❌ Request cancelled by context", zap.String("method", method), zap.Error(ctx.Err()))
		return nil, ctx.Err()
