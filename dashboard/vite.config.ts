  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // Proxy alerts endpoint to the backend server (rewrite to include /api/v1 prefix)
        '/alerts': {
          target: backendHttpTarget,
          changeOrigin: true,
          secure: false,
          rewrite: (path) => path.replace(/^\/alerts/, '/api/v1/alerts'),
        },
        // Proxy alert-types endpoint to the backend server (rewrite to include /api/v1 prefix)
        '/alert-types': {
          target: backendHttpTarget,
          changeOrigin: true,
          secure: false,
          rewrite: (path) => path.replace(/^\/alert-types/, '/api/v1/alert-types'),
        },
        ...
        '/session-id': {
          ...
          rewrite: (path) => path.replace(/^\/session-id/, '/api/v1/session-id'),
        },
