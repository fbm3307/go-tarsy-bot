    ...
    try {
      ...
      if (applyFilters && (
        ...
        const historicalFilters: SessionFilter = {
          ...filters,
          // For historical view, include all statuses by default unless specific status filter is applied
          status: filters.status && filters.status.length > 0
            ? filters.status
            : ['completed', 'failed', 'in_progress', 'pending'] as ('completed' | 'failed' | 'in_progress' | 'pending')[]
        };
